...
const StaffLoginScreen: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const savedEmail = await AsyncStorage.getItem('staff_saved_email');
        const savedPassword = Platform.OS !== 'web'
          ? await SecureStore.getItemAsync('staff_saved_password')
          : await AsyncStorage.getItem('staff_saved_password');
        const autoLogin = await AsyncStorage.getItem('staff_auto_login');
        ...
        if (savedEmail && savedPassword) {
          ...
          if (autoLogin === 'true') {
            ...
            try {
              ...
              // If the auto-login resolved to a non-staff role, sign out (wrong portal)
              const autoRole = response?.session?.validatedUser?.role?.code;
              if (response?.session && !isStaffLoginAllowedRole(autoRole)) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const validatedUser = response.session.validatedUser;
      const userRole = validatedUser?.role?.code;

      if (!validatedUser || !userRole) {
        showAlert({ type: 'error', title: 'Login Failed', message: 'Invalid credentials' });
        await AuthService.signOut();
        return;
      }

      if (isStaffLoginAllowedRole(userRole)) {
        if (rememberMe) {
          await AsyncStorage.setItem('staff_saved_email', email);
          Platform.OS !== 'web'
            ? await SecureStore.setItemAsync('staff_saved_password', password)
            : await AsyncStorage.setItem('staff_saved_password', password);
          await AsyncStorage.setItem('staff_auto_login', 'true');
        } else {
          await AsyncStorage.removeItem('staff_saved_email');
          Platform.OS !== 'web'
            ? await SecureStore.deleteItemAsync('staff_saved_password')
            : await AsyncStorage.removeItem('staff_saved_password');
        ...
        showAlert({
          ...
          message: 'This login is for staff only.',
        });
        await AuthService.signOut();
      }
    } catch (err: any) {
          ...
          <View style={styles.body}>
            ...
            >
              ...
              <View style={styles.fieldGap}>
                <FloatingInput
                  label={t('emailAddress') || 'Staff Email'}
                  value={email}
