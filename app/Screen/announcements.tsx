...
const DateCol = memo(function DateCol({ item, styles }: { item: UINotice, styles: any }) {
  const parts = useMemo(() => {
...
function AnnouncementsScreenInner() { // OPT: Wrapped by ErrorBoundary in default export.
  ...
  const renderItem = useCallback( // OPT: Stable FlatList renderItem.
    ({ item, index }: { item: UINotice; index: number }) => <NoticeTimelineRow item={item} index={index} styles={styles} onMessagePress={noopMessagePress} />, // OPT: Delegate to memo row.
...
const getStyles = (theme: SchoolTheme, isDark: boolean) => {
  ...
  return StyleSheet.create({
    ...
    iconBox: {
      ...
      marginRight: 14,
      borderWidth: 1,
      backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : '#EEF2FF',
      borderColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(99,102,241,0.08)'
    },
    pageTitle: { fontSize: 18, fontWeight: '800', letterSpacing: -0.2, color: isDark ? '#F8FAFC' : '#1E293B' },
    subtitle: { fontSize: 12.5, marginTop: 2, color: isDark ? '#94A3B8' : '#64748B' },
