...
export default function AdminDiaryViewerScreen() {
  ...
  // Fetch entries whenever class/section changes or on mount
  const fetchEntries = async () => {
    ...
    try {
      ...
      alertCompat('Error', err.message || 'Failed to fetch diary entries');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <View style={[styles.container, { backgroundColor: pageBg }]}>
      <AdminHeader
        ...
        rightAction={{
          ...
          onPress: () => router.push('/admin/diary/history'),
        }}
      />
      ...
      <View style={styles.filterContainer}>
        ...
        {/* Section Selection (only enabled if Class is selected, or we show all) */}
        <View style={styles.filterRow}>
      ...
      >
        {loading && !refreshing ? (
          ...
          <Animated.View entering={ZoomIn} style={styles.emptyBox}>
            <Ionicons name="book-outline" size={48} color={subColor} style={{ opacity: 0.5 }} />
            <Text style={[styles.emptyTitle, { color: titleColor }]}>No Diary Entries</Text>
            <Text style={[styles.emptySub, { color: subColor }]}>There are no entries posted for today in this class.</Text>
          </Animated.View>
        ) : (
